...
public abstract class QueryAbstract {
   ...
   *
   * @param username   new user's username. User names are unique within the
   *                   system.
  ...
  /**
   * Searches for flights, according to user-specified origin, destination, and
   * other parameters.
   *
   * Searches for flights from the given origin city to the given destination
   * city, on the given
   * day of the month. If {@code directFlight} is true, it only searches for
   * direct flights,
   * otherwise is searches for direct flights and flights with two "hops." Only
   * searches for up
   ...
   *         {@code Flight} class.
   *         Itinerary numbers in each search should always start from 0 and
   *         increase by 1.
   *
   ...
   *         not logged
   *         in\n". If the user is trying to book an itinerary with an invalid ID
   *         or without
   *         having done a search, then return "No such itinerary
   *         {@code itineraryId}\n". If the
   *         user already has a reservation on the same day as the one that they
   *         are trying to
   *         book now, then return "You cannot book two flights in the same
   *         day\n". For all
   *         other errors, return "Booking failed\n".
   *
   *         If booking succeeds, return "Booked flight(s), reservation ID:
   *         [reservationId]\n"
   *         where reservationId is a unique number in the reservation system that
   *         starts from
   *         1 and increments by 1 each time a successful reservation is made by
   *         any user in
   *         the system.
   *
   * @see #search()
   ...
   *         "Cannot find unpaid reservation [reservationId] under user:
   *         [username]\n".
   ...
   *         balance:
   *         [balance]\n" where [balance] is the remaining balance in the user's
   *         account.
   *
   * @see #book()
   */
